...
import { formatTeacherReport } from './report';
...
type ViewMessage = { command: 'newQuestion' | 'analyze' | 'hint' | 'runTests' | 'submit' };
interface AnalysisUiResult { ok: boolean; message: string; feedback?: string; }
interface BaseProgram { workspace: string; pythonFile?: string; }

class BuildMyLogicViewProvider implements vscode.WebviewViewProvider {
	private view: vscode.WebviewView | undefined;
	private readonly questions = new QuestionSession(new LocalQuestionProvider());
	private readonly feedback = new LocalFeedbackProvider();

	public constructor(private readonly analyseCode: () => Promise<AnalysisUiResult>) {}
		...
		view.webview.onDidReceiveMessage(async (message: unknown) => {
			...
			try {
				switch (message.command) {
					case 'newQuestion': this.updateQuestion(await this.questions.next()); break;
					case 'analyze': { const result = await this.analyseCode(); this.post({ type: 'feedback', text: result.feedback ?? result.message, level: result.ok ? 'feedback' : 'error' }); break; }
					case 'hint': await this.showHint(); break;
					case 'runTests': this.post({ type: 'tests', passed: 4, total: 4 }); this.postStatus('Local demo tests passed. Real Python test execution will be connected later.', 'info'); break;
					case 'submit': this.postStatus('GitHub and website submission will be implemented later.', 'info'); break;
				}
			} catch (error) {
	...
	private async showHint(): Promise<void> {
		...
		if (!question) { this.postStatus('No question loaded yet.', 'error'); return; }
		const result = await this.feedback.generateHint(question);
		if (result.ok) { this.post({ type: 'feedback', text: 'Hint: ' + result.value.text, level: 'hint' }); }
		else { this.postStatus(result.error.message, 'error'); }
	}
	private updateQuestion(result: ProviderResponse<Question | undefined>): void {
		if (!result.ok) { this.postStatus(result.error.message, 'error'); return; }
		this.post({ type: 'question', question: result.value });
	}
	private postStatus(text: string, level: 'info' | 'error'): void { this.post({ type: 'status', text, level }); }
...
function isViewMessage(value: unknown): value is ViewMessage { return typeof value === 'object' && value !== null && 'command' in value && ['newQuestion', 'analyze', 'hint', 'runTests', 'submit'].includes(String(value.command)); }
async function findBaseProgram(): Promise<BaseProgram> {
	const folder = vscode.workspace.workspaceFolders?.[0];
	if (!folder) { return { workspace: 'No workspace open' }; }
	const editor = vscode.window.activeTextEditor;
	if (editor?.document.languageId === 'python') { return { workspace: folder.name, pythonFile: editor.document.uri.fsPath }; }
...
export function activate(context: vscode.ExtensionContext): void {
	...
	const analyze = async (): Promise<AnalysisUiResult> => {
		try {
			const editor = vscode.window.activeTextEditor;
			if (!editor || editor.selection.isEmpty) { const message = 'Select Python code first, then choose Analyze My Code.'; void vscode.window.showWarningMessage('BuildMyLogic: ' + message); return { ok: false, message }; }
			const selected = editor.document.getText(editor.selection);
			if (editor.document.languageId !== 'python' && !looksLikePython(selected)) { const message = 'The local analyser currently supports selected Python code.'; void vscode.window.showWarningMessage('BuildMyLogic: ' + message); return { ok: false, message }; }
			const result = await feedback.analyzeCode(selected);
			if (!result.ok) { void vscode.window.showErrorMessage(result.error.message); return { ok: false, message: result.error.message }; }
			...
			output.clear(); output.appendLine(report); output.show(true);
			return { ok: true, message: result.value.summary, feedback: report };
		} catch (error) { console.error('BuildMyLogic analysis failed.', error); return { ok: false, message: 'BuildMyLogic could not analyse this selection. Please try again.' }; }
	};
